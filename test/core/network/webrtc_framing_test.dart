...
void main() {
  group('WebRTC DataChannel 5-byte Framing Tests', () {
    test(
      'frameMessage creates correct 5-byte header [flag, len_b3, len_b2, len_b1, len_b0]',
      () {
  ...
  group('FrameAccumulator SCTP Fragmentation & Sticky Packets Tests', () {
    ...
    test('reconstructs frame fragmented across 3 SCTP chunks', () {
      ...
      final payload = Uint8List.fromList(
        utf8.encode('fragmented SCTP message across chunks'),
      );
      ...
      expect(
        utf8.decode(drained.first.payload),
        'fragmented SCTP message across chunks',
      );
      expect(accumulator.bufferedBytes, 0);
    });

    test(
      'correctly handles sticky packets with multiple concatenated frames',
      () {
        final accumulator = FrameAccumulator();
        final f1 = WebRtcMeshClient.frameMessage(
          Uint8List.fromList(utf8.encode('frame-1')),
          flag: 0x01,
        );
        final f2 = WebRtcMeshClient.frameMessage(
          Uint8List.fromList(utf8.encode('frame-2-larger-payload')),
          flag: 0x02,
        );
        final f3 = WebRtcMeshClient.frameMessage(
          Uint8List.fromList(utf8.encode('frame-3')),
          flag: 0x00,
        );
    ...
    test(
      'preserves partial frame in buffer when sticky chunk has incomplete tail',
      () {
        final accumulator = FrameAccumulator();
        final f1 = WebRtcMeshClient.frameMessage(
          Uint8List.fromList(utf8.encode('complete-frame')),
          flag: 0x01,
        );
        final f2 = WebRtcMeshClient.frameMessage(
          Uint8List.fromList(utf8.encode('tail-frame')),
          flag: 0x02,
        );
