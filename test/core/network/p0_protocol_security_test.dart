  ...
  Future<Uint8List> callUnary(String rpcPath, Uint8List payload) async {
    ...
    if (shouldThrowOnCall) {
      throw InFlightRpcException(
        ...
        cause:
            errorToThrow ??
            TimeoutException('P2P DataChannel timeout on $rpcPath'),
      );
    }
...
void main() {
  group('P0 #10: 認證狀態過早放行與未認證封包阻絕測試 (Authentication Gating Tests)', () {
    test(
      'drops all unauthenticated business frames prior to Channel Binding ACK',
      () async {
        ...
        final cascadeSub = client.cascadeStreamForTesting.listen(
          cascadeChunks.add,
        );
        final terminalSub = client.terminalStreamForTesting.listen(
          terminalChunks.add,
        );
        ...
        final fakeCascadeFrame = FramePacket(
          ...
          payload: Uint8List.fromList(
            utf8.encode('{"thinking":"injected unauthenticated prompt"}'),
          ),
        );
        ...
        final fakeTerminalFrame = FramePacket(
          flag: 0x02,
          payload: Uint8List.fromList(
            utf8.encode('rm -rf / # unauthenticated terminal exploit\n'),
          ),
        );
        ...
        final fakeRpcFrame = FramePacket(
          flag: 0x00,
          payload: Uint8List.fromList(
            utf8.encode('{"request_id":1,"payload":"pwned"}'),
          ),
        );
    ...
    test(
      'transitions to authenticated ONLY after receiving Channel Binding ACK',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
        ...
        final ackFrame = FramePacket(
          ...
          payload: Uint8List.fromList(
            utf8.encode(
              jsonEncode({'type': 'channel_binding_ack', 'status': 'OK'}),
            ),
          ),
        );
        ...
        final legitCascadeFrame = FramePacket(
          flag: 0x01,
          payload: Uint8List.fromList(
            utf8.encode('{"thinking":"authenticated thinking"}'),
          ),
        );
        ...
        expect(
          utf8.decode(cascadeChunks.first),
          contains('authenticated thinking'),
        );
      },
    );

    test(
      'strictly rejects Channel Binding ACK if received on auxiliary channel or with non-zero flag',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
        ...
        await client.routeFrameForTesting(
          challengeFrame,
          fromChannel: 'proxy-channel',
        );
        ...
        final spoofAckCascade = FramePacket(
          flag: 0x00,
          payload: Uint8List.fromList(
            utf8.encode(jsonEncode({'type': 'channel_binding_ack'})),
          ),
        );
        await client.routeFrameForTesting(
          spoofAckCascade,
        ...
        expect(
          client.isChannelAuthenticatedForTesting,
          isFalse,
          reason: '不得接受來自 cascade-channel 的 ACK',
        );
        ...
        expect(
          client.isChannelAuthenticatedForTesting,
          isFalse,
          reason: '不得接受帶有非 0x00 旗標的 ACK',
        );
      },
    );

    test(
      'strictly rejects unsolicited Channel Binding ACK if no challenge nonce was ever received',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
    ...
    test(
      'disconnects and resets auth state upon receiving channel_binding_rejected or error',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
        ...
        final challengeFrame = FramePacket(
          flag: 0x00,
          payload: Uint8List.fromList(
            utf8.encode(jsonEncode({'challenge_nonce': 'nonce-reject-test'})),
          ),
        );
        await client.routeFrameForTesting(
          challengeFrame,
  ...
  group('P0 #2: Request ID 配對、防 FIFO 盲配與串流隔離測試', () {
    test(
      'correctly matches out-of-order unary RPC responses by request_id without FIFO mismatch',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
            ...
            utf8.encode(
              jsonEncode({
                ...
                'payload': base64Encode(
                  utf8.encode('Response For Request 102'),
                ),
              }),
            ),
          ),
          ...
          payload: Uint8List.fromList(
            utf8.encode(
              jsonEncode({
                'request_id': 101,
                'payload': base64Encode(
                  utf8.encode('Response For Request 101'),
                ),
              }),
            ),
          ),
        ...
        expect(
          completerReq2.isCompleted,
          isTrue,
          reason: 'Request 102 應已依 request_id 精確完成',
        );
        expect(
          completerReq1.isCompleted,
          isFalse,
          reason: 'Request 101 絕不可被 FIFO 盲配搶先完成',
        );
    ...
    test(
      'drops response frames with missing or unmatched request_id without popping FIFO queue',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
        ...
        final noIdFrame = FramePacket(
          flag: 0x00,
          payload: Uint8List.fromList(
            utf8.encode('{"payload":"raw response"}'),
          ),
        );
        ...
        final wrongIdFrame = FramePacket(
          flag: 0x00,
          payload: Uint8List.fromList(
            utf8.encode('{"request_id":999,"payload":"other"}'),
          ),
        );
    ...
    test(
      'strictly isolates Terminal and Cascade streams and never leaks RPC frames into Terminal',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
    ...
    test(
      'completes with RpcException when remote host returns error payload (P0 #2 & P0 #4)',
      () async {
        final client = WebRtcMeshClient(
          baseUrl: 'https://mock.googleapis.com',
          googleAccessToken: 'mock-token',
  ...
  group('P0 #3: 寫入重複執行防禦測試 (Duplicate Write on Relay Retry Tests)', () {
    test(
      'blocks relay retry and throws DuplicateExecutionPreventedException when in-flight non-idempotent RPC fails',
      () async {
        ...
        for (final rpc in nonIdempotentRpcs) {
          ...
          expect(
            () => manager.callUnary(
              rpc,
              Uint8List.fromList(utf8.encode('{"cmd":"test"}')),
            ),
    ...
    test(
      'allows safe relay fallback for idempotent query RPCs when P2P fails in-flight',
      () async {
        final fakeRelay = FakeRelayForP0Test();
        final fakeMesh = FakeMeshForP0Test();
        fakeMesh.isP2pOpen = true;
    ...
    test(
      'routes directly to relay when P2P was offline from start (not a retry)',
      () async {
        final fakeRelay = FakeRelayForP0Test();
        final fakeMesh = FakeMeshForP0Test();
    ...
    test(
      'allows safe relay fallback for non-idempotent RPC when error is PreFlightException (never sent onto wire, P0 #3)',
      () async {
        final fakeRelay = FakeRelayForP0Test();
        final fakeMesh = FakeMeshForP0Test();
        fakeMesh.isP2pOpen = true;
        fakeMesh.throwPreFlight =
            true; // 模擬封包根本未送出至網路線路 (例如 DataChannel 未開啟或緩衝異常)
        ...
        expect(
          fakeRelay.callCount,
          1,
          reason: '根本未送出的非冪等請求安全 Fallback 至 Relay',
        );
        expect(manager.currentTransport, TransportType.relay);
      },
    );
