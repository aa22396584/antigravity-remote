...
void main() {
  group('RemoteControlService Real Test Harness Tests', () {
    ...
    test(
      'Demo Mode sendTerminalInput does NOT duplicate user input (fixes Double Echo)',
      () async {
        final service = RemoteControlService(isDemoMode: true);
        addTearDown(service.dispose);
        ...
        expect(
          chunks.any((c) => c.text.contains('antigravity_remote')),
          isTrue,
        );
      },
    );
    ...
    test(
      'Live Mode calls SendUserCascadeMessage and starts reactive stream',
      () async {
        ...
        expect(
          harness.streamCalls.any(
            (s) => s.rpcPath == ApiEndpoints.streamCascadeReactiveUpdates,
          ),
          isTrue,
        );
      },
    ...
    test(
      'Live Mode parses 5-byte framed binary stream thinking updates',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'Live Mode parses 5-byte framed binary tool steps and snake_case properties',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
        ...
        harness.emitFramedCascadeJson({
          'trajectory_step': {
            ...
            'output':
                'Found 1 match in lib/core/network/webrtc_mesh_client.dart',
            'status': 'completed',
        ...
        expect(
          step.output,
          contains('lib/core/network/webrtc_mesh_client.dart'),
        );
    ...
    test(
      'Live Mode parses 5-byte framed binary code diffs in file changes',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'Live Mode parses UserInteraction permission request and executes handleApproval',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
        ...
        expect(
          harness.unaryCalls.length,
          2,
        ); // sendUserCascadeMessage + handleCascadeUserInteraction
        ...
        expect(
          approvalCall.payloadJson['userFeedback'],
          'Approve flutter test execution',
        );
      },
    );

    test(
      'Live Mode accumulates text streaming and finalizes on is_final',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'Cascade and Terminal streams are isolated without cross-contamination',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'Live Mode handleApproval sends DECISION_REJECT when approved is false',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'Live Mode updates TrajectoryStep in-place when duplicate step_id arrives',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
        ...
        expect(messages.last.trajectorySteps.length, 1);
        expect(
          messages.last.trajectorySteps.first.status,
    ...
    test(
      'Live Terminal stream strips 5-byte framing header from incoming chunks',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'Live Mode cancelTask sends cancelCascadeTask and stops active stream',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
        ...
        expect(
          harness.unaryCalls.any(
            (c) => c.rpcPath == ApiEndpoints.cancelCascadeTask,
          ),
          isTrue,
        );
        expect(messages.last.isStreaming, isFalse);
    ...
    test(
      'Live Terminal stream uses Utf8ChunkDecoder to decode split Chinese character across packets',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
