...
class TrackableDualTransportManager extends DualTransportManager {
  ...
  TrackableDualTransportManager({required this.deviceTag, this.onConnectAll})
    : super(
...
void main() {
  group('P0 #1: 錯機控制防禦與裝置切換一致性測試 (Target Consistency Tests)', () {
    test(
      'switching active device clears terminal chunks from old device',
      () async {
        ...
        await deviceNotifier.addDevice(
          instanceId: devA.instanceId,
          name: devA.name,
        );
        await deviceNotifier.addDevice(
          instanceId: devB.instanceId,
          name: devB.name,
        );
        ...
        terminalNotifier.state = terminalNotifier.state.copyWith(
          chunks: [
            TerminalChunk(
              text: 'Alpha-Host-Prompt> ls -la\n',
              timestamp: DateTime.now(),
            ),
            TerminalChunk(
              text: 'secret_file_on_alpha.txt\n',
              timestamp: DateTime.now(),
            ),
          ],
        );
        ...
        expect(
          ...
          isEmpty,
          reason: '切換設備時必須清理舊終端緩衝，避免誤導控制目標',
        );
      },
    );

    test(
      'concurrent device switching drops outdated in-flight connections (Epoch / Generation check)',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        await deviceNotifier.addDevice(
          instanceId: dev1.instanceId,
          name: dev1.name,
        );
        await deviceNotifier.addDevice(
          instanceId: dev2.instanceId,
          name: dev2.name,
        );
        await deviceNotifier.addDevice(
          instanceId: dev3.instanceId,
          name: dev3.name,
        );
    ...
    test(
      'removing active device disposes transport and cleans up session completely',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        await deviceNotifier.addDevice(
          instanceId: devOnly.instanceId,
          name: devOnly.name,
        );
        await deviceNotifier.selectDevice(devOnly);

        expect(
          container.read(deviceProvider).activeDevice?.instanceId,
          devOnly.instanceId,
        );
    ...
    test(
      'Live Mode: switching active device completely disposes old transport manager',
      () async {
        ...
        final container = ProviderContainer(
          overrides: [
            transportManagerFactoryProvider.overrideWithValue(({
              ...
              final tag = relayClient.targetInstanceUuid.replaceAll(
                'uuid-',
                '',
              );
        ...
        expect(
          createdManagers['live-a']!.isDisposed,
          isTrue,
          reason: '切換設備時舊連線必須立即且完整 dispose',
        );
        expect(
          createdManagers['live-b']!.isDisposed,
          isFalse,
          reason: '新連線必須維持開啟狀態',
        );
    ...
    test(
      'Live Mode: removing active device disposes its transport manager',
      () async {
        final createdManagers = <String, TrackableDualTransportManager>{};
        ...
        expect(
          createdManagers['dev-live-solo']!.isDisposed,
          isTrue,
          reason: '刪除裝置必須銷毀其 Transport',
        );
        expect(notifier.transportManager, isNull);
        expect(container.read(deviceProvider).activeDevice, isNull);
        expect(
          container.read(deviceProvider).activeTransport,
    ...
    test(
      'Live Mode: connectAll error handles gracefully, disposes failed manager and transitions to offline',
      () async {
        ...
        expect(
          failedMgr!.isDisposed,
          isTrue,
          reason: '連線失敗時新 Manager 必須被 dispose 避免資源洩漏',
        );
        expect(notifier.transportManager, isNull);
        final state = container.read(deviceProvider);
        expect(state.isConnecting, isFalse);
