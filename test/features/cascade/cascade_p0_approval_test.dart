...
void main() {
  group('P0 #4: 審批過早顯示成功防禦測試 (Premature Approval Success Prevention Tests)', () {
    test(
      'retains pending interaction and preserves step status when remote approval RPC fails',
      () async {
        ...
        expect(
          currentState.pendingInteraction,
          isNotNull,
          reason: '遠端失敗時必須保留待審批狀態',
        );
        expect(
          currentState.pendingInteraction?.interactionId,
          'req-critical-rm',
        );
        ...
        expect(
          currentStep.status,
          StepStatus.waitingUserInteraction,
          reason: '遠端未確認前絕不可標註為 completed',
        );
    ...
    test(
      'updates step status to completed and clears pending interaction ONLY when remote RPC confirms success',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'retains pending interaction and preserves step status when remote approval RPC fails with RpcException',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);

        // 模擬對端遠端主機回傳明確業務拒絕錯誤 (例如 403 Forbidden 或 reject)
        harness.unaryError = const RpcException(
          'Permission denied by remote agent',
          statusCode: 403,
        );
        ...
        expect(
          state.messages.first.trajectorySteps.first.status,
          StepStatus.waitingUserInteraction,
        );
    ...
    test(
      'prevents concurrent handleApproval calls for the same interactionId (throws StateError)',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
    ...
    test(
      'does not clear pendingInteraction if a different interactionId completes',
      () async {
        final harness = MockTransportHarness();
        addTearDown(harness.dispose);
  ...
  group('P0 #1: 裝置切換與畫面目標一致性測試 (Target Inconsistency on Device Switch)', () {
    test(
      'switching active device clears old conversation and pending approvals',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        expect(
          ...
          isNull,
          reason: '切換設備時必須清理舊機器之待審批項，避免錯批到新機器',
        );
      },
    );
  });
