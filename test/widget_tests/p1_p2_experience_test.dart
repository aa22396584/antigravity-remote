...
''';
    ...
    testWidgets('ChatInputBar restores draft when onSend returns false or throws exception', (tester) async {
    ...
    testWidgets('CodeDiffViewer auto-detects file name from diff headers when fileName is not provided', (tester) async {
...
''';
    ...
    testWidgets('UserApprovalDialog renders embedded CodeDiffViewer for unified diffs and copy button >=44x44', (tester) async {
      const diffContent = '''
--- a/test.dart
+++ b/test.dart
@@ -1,2 +1,3 @@
...
''';
      ...
      final copyBtn = tester.widget<IconButton>(find.byWidgetPredicate(
    ...
    testWidgets('TerminalConsoleCard touch targets meet >= 44x44 and adapts to keyboard insets', (tester) async {
      final chunks = [
        TerminalChunk(
      ...
      final sendBtn = tester.widget<IconButton>(find.byWidgetPredicate(
        (w) => w is IconButton && w.tooltip == '發送按鍵',
      ));
      ...
      final clearBtn = tester.widget<IconButton>(find.byWidgetPredicate(
      ...
      final ctrlCContainer = tester.widget<Container>(find.ancestor(
        of: find.text('Ctrl+C'),
        matching: find.byType(Container),
      ).first);
      expect(ctrlCContainer.constraints?.minWidth, greaterThanOrEqualTo(44.0));
      expect(ctrlCContainer.constraints?.minHeight, greaterThanOrEqualTo(44.0));

      expect(find.text('Ctrl+D'), findsOneWidget);
      expect(find.text('Esc'), findsOneWidget);
    });

    testWidgets('CascadeChatView renders failed user message with retry and edit draft >=44x44', (tester) async {
      ...
      final container = ProviderContainer(
        overrides: [
          cascadeProvider.overrideWith(() => MockCascadeNotifier([failedMsg])),
        ],
      );
