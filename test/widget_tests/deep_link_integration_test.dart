...
void main() {
  group('Deep Link Integration Tests', () {
    ...
    testWidgets('incoming deep link while modal route is open pops back to root workspace', (tester) async {
      final deepLinkService = DeepLinkService();
      ...
      expect(find.textContaining('Antigravity (user@test.io)'), findsOneWidget);

      deepLinkService.dispose();
    });

    testWidgets('in-app QR pairing via manual input binds device, sets email name, and switches to workspace', (tester) async {
      await tester.pumpWidget(
        const ProviderScope(
      ...
      expect(find.textContaining('Antigravity (engineer@google.com)'), findsOneWidget);
